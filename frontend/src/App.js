    ...
    <section className="hero"><div className="container-x hero-grid">
      <div>
        ...
        <div style={{marginTop:"1.5rem",display:"flex",gap:"0.5rem",flexWrap:"wrap"}}>
          {["Detached","Luxury","Equestrian","Estate Sales","Condos"].map(s => <span key={s} className="pill">{s}</span>)}
        </div>
      </div>
...
const GlossaryTerm = () => {
  ...
  return (<section className="section"><div className="container-x" style={{maxWidth:"48rem"}}>
    ...
    <h1 className="section-title">{t.term}</h1>
    <p style={{fontFamily:"Inter,sans-serif",fontSize:"1.05rem",lineHeight:1.75,color:"var(--ink)"}}>{t.definition}</p>
    ...
    <div className="notice" style={{marginTop:"2rem"}}>This is general information only. For advice specific to your situation, consult a licensed REALTOR®.</div>
    {t.faqs && t.faqs.length>0 && <script type="application/ld+json" dangerouslySetInnerHTML={{__html:JSON.stringify({"@context":"https://schema.org","@type":"FAQPage","mainEntity":t.faqs.map(f=>({"@type":"Question","name":f.q,"acceptedAnswer":{"@type":"Answer","text":f.a}}))})}}/>}
  </div></section>);
};

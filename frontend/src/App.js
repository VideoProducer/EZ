...
const DOOGIE_POINT_R = "/images/doogie/pointing-right-transparent.webp";
  ...
  <footer><div className="container-x">
    <div className="footer-grid">
      ...
      <div><h4>Consumer Protection</h4><ul>
        ...
        <li><Link to="/code-of-ethics" data-testid="footer-code-of-ethics">REALTOR® Code of Ethics</Link></li>
        <li><a href="https://www.gvrealtors.ca" target="_blank" rel="noopener noreferrer" data-testid="footer-gvr-link">Greater Vancouver REALTORS® (GVR) ↗</a></li>
      </ul>
      <p data-testid="footer-board-membership" style={{fontSize:"0.72rem",lineHeight:1.5,opacity:0.75,marginTop:"0.75rem",fontStyle:"italic"}}>
        Doug LeMaire, REALTOR® is a member in good standing of <strong>Greater Vancouver REALTORS® (GVR)</strong> and the <strong>Canadian Real Estate Association (CREA)</strong>, bound by the <Link to="/code-of-ethics" style={{color:"var(--brand-gold)"}}>REALTOR® Code of Ethics</Link>.
      </p></div>
      <div><h4>Contact</h4>
...
const GlossaryTerm = () => {
  ...
  // FAQPage schema — Answer.citation added Feb 2026 (SEO audit #5).  Every
  // answer points to the canonical EZtoFind glossary entry as its
  // CreativeWork source; the entry itself embeds the primary BC statute.
  // This is the AEG signal Google + Perplexity look for when deciding
  ...
  const faqSchema = (t.faqs && t.faqs.length>0) ? {
    ...
    "mainEntity": t.faqs.map(f => ({
      ...
      "acceptedAnswer":{
        ...
        "citation":{
          "@type":"CreativeWork",
          "name":`${t.term} — BC real estate glossary (EZtoFind.ca)`,
          "url":`https://eztofind.ca/glossary/${t.slug}`,
          "inLanguage":"en-CA",
          "author":{"@type":"Person","name":"Doug LeMaire, REALTOR®"},
          "publisher":{"@type":"Organization","name":"EZtoFind.ca","url":"https://eztofind.ca"}
        }
      }
    }))
  ...
  return (<section className="section" dir={rtl?"rtl":"ltr"}><div className="container-x" style={{maxWidth:"42rem"}}>
    ...
    <form onSubmit={submit} className="paper" data-testid="buyer-form">
      ...
      {/* CASL marketing-consent card + BCFSA DoRTS checkbox removed per Feb 2026
          form-shortening directive. DoRTS is presented at first substantive
          contact (before formal representation), not at lead-form stage. */}
...
const SellerForm = () => {
  ...
  return (<section className="section" dir={rtl?"rtl":"ltr"}><div className="container-x" style={{maxWidth:"42rem"}}>
    ...
    <form onSubmit={submit} className="paper" data-testid="seller-form">
      ...
      {/* CASL + DoRTS checkboxes removed per Feb 2026 form-shortening directive. */}
  ...
  return (
    <section className="section"><div className="container-x" style={{maxWidth:"46rem",fontFamily:"Inter,sans-serif",lineHeight:1.75,color:"var(--ink)"}}>
      ...
      <ul>
        ...
        <li><strong>Destruction or anonymization when no longer needed:</strong> Under PIPA s.35, we destroy or anonymize personal information as soon as (a) the purpose it was collected for has been fulfilled AND (b) retention is no longer required for legal or business reasons. You may also <strong>withdraw consent</strong> at any time (subject to legal or contractual restrictions that may apply on a case-by-case basis) and request that we assess whether the collection purpose is exhausted so retention is no longer required. Please note that PIPA does not create an unconditional "right to erasure" equivalent to the EU GDPR — where an active legal, brokerage, or business obligation exists (for example, a BCFSA-required brokerage transaction record under RESA, or a live client-lifecycle relationship), that record must be retained until the obligation lapses.</li>
...
const Terms = () => <Legal title="Terms of Use" body={<>
  ...
  <p>Questions about these Terms: <a href="mailto:info@eztofind.ca" style={{color:"var(--brand-blue)"}}>info@eztofind.ca</a>. Privacy questions: <a href="mailto:info@eztofind.ca" style={{color:"var(--brand-blue)"}}>info@eztofind.ca</a>. Complaints or regulatory concerns: see our <Link to="/complaints" style={{color:"var(--brand-blue)"}}>Complaints page</Link>.</p>
</>}/>;
  ...
  return <AdminShell active="reset">
    ...
    <div style={{background:"#FFF3E0",border:"2px solid #F5A623",borderRadius:8,padding:"1.25rem",marginBottom:"2rem"}}>
      ...
      <div style={{marginTop:"0.5rem",fontSize:"0.9rem",lineHeight:1.55,color:"var(--ink)"}}>
        Real buyer/seller/valuation leads and REALTOR® applications are subject to <strong>PIPA s.35 purpose-based retention</strong> — and, where a form submission progresses into an active brokerage transaction with Doug, the resulting transaction record carries a <strong>minimum 7-year BCFSA / RESA retention</strong>. Once you flip DNS to your live domain, individual records should be deleted only through the automatic daily retention purger or a documented DSAR / withdrawal-of-consent request — never through this button. Every reset writes a permanent attestation to <code>retention_purge_log</code> for audit purposes.
      </div>
    </div>
...
const AdminEmailLog = () => {
  ...
  return <AdminShell active="email-log">
    ...
    <p style={{color:"var(--muted)"}}>Audit trail of every reminder email queued or sent. Consent metadata is retained for a minimum of 3 years per CASL; where a reminder relates to an active brokerage transaction, the associated transaction record is also retained for a minimum of 7 years per BCFSA / RESA.</p>
...
const CommunityPage = () => {
  ...
  //
  // ── FAQ WORDING VARIANCE (Feb 8, 2026) ───────────────────────────────
  // Answers used to share identical wording across all 240 BC communities,
  // which triggered Google's near-duplicate cluster and suppressed most
  // communities from SERPs.  We now pick 1 of 3 wordings per question
  // deterministically per community — same page every time for that slug
  // (cache-friendly) but ~3^20 = ~3.5 billion possible combinations across
  // the site so no two community FAQs look like textual siblings to a
  // search engine.
  //
  // The seed is `${slug}::${index}` — stable per community per question.
  // FNV-1a 32-bit hash gives even 3-way distribution with zero deps.
  const _pickVariant = (seed, options) => {
    let h = 2166136261;
  ...
  const _v = (i, options) => _pickVariant(`${slug}::${i}`, options);
  ...
  const communityFaqLd = (found && syn?.synopsis) ? {
    ...
    "mainEntity":[
      ...
      {"@type":"Question","name":_v(6,[`Are ${found}, BC homes a good investment?`,`Should I buy a home in ${found} for investment purposes?`,`Is ${found} real estate a smart place to put money?`]),"acceptedAnswer":{"@type":"Answer","text":_v(106,[`BCFSA and CREA prohibit REALTORS® from guaranteeing investment outcomes, so I can't answer that as a recommendation. What EZtoFind.ca can share is factual: the current median list price for ${found}, its active-listing count, and how ${found} compares to nearby communities — all visible on the ${found} community page.`,`That's the kind of question a REALTOR® can't answer with a "yes" or "no" — BCFSA rules forbid it. What EZtoFind.ca does show, on the ${found} community page, is the current median list price and the active-inventory count so you can compare ${found} against nearby communities and form your own view.`,`Compliance rules stop me from calling any property or neighbourhood a "good investment." What I'll share is objective data — the median list price in ${found} today, the number of active listings, and the geographic context (nearby communities are one click away from the ${found} page). Use those inputs alongside your own goals and, ideally, an independent financial advisor.`]),"citation":{"@type":"CreativeWork","name":"BCFSA Real Estate Rules — Prohibition on Investment Guarantees","url":"https://www.bcfsa.ca/industry-resources/real-estate-professional-resources/knowledge-base"}}},
      {"@type":"Question","name":_v(7,[`How long is the commute from ${found} to downtown Vancouver?`,`What's the drive time from ${found} to Vancouver?`,`How connected is ${found} to Metro Vancouver's transit network?`]),"acceptedAnswer":{"@type":"Answer","text":_v(107,[`Drive time from ${found} to downtown Vancouver varies with time of day and route (Highway 1, Highway 7, or a SkyTrain-adjacent park-and-ride). The ${found} community page lists the nearest SkyTrain stations, West Coast Express stops, and highway on-ramps so you can estimate your specific commute before viewing homes.`,`Commutes from ${found} depend on the corridor — Highway 1 vs Highway 7 vs a transit connection. Check the ${found} page on EZtoFind.ca for a summary of nearest transit stations and highway access points; that gives you the fastest way to sanity-check drive times against your own workplace.`,`Getting from ${found} to downtown Vancouver is a mix of highway + transit depending on where you live within the community. On the ${found} page you'll see the closest SkyTrain / West Coast Express station, the nearest Highway 1 or 7 on-ramp, and any park-and-ride facilities.`]),"citation":{"@type":"CreativeWork","name":"TransLink — Metro Vancouver Transit","url":"https://www.translink.ca"}}},
      {"@type":"Question","name":_v(8,[`What schools serve ${found}, BC?`,`Which school district covers ${found}?`,`How are the schools in ${found}, British Columbia?`]),"acceptedAnswer":{"@type":"Answer","text":_v(108,[`${found} is served by the local School District listed on the BC Ministry of Education directory. Catchment boundaries shift periodically, so confirm the current catchment with the school district before making an offer — a home's school catchment can materially affect resale value.`,`The BC Ministry of Education publishes ${found}'s school district assignment. Because catchments are redrawn from time to time, verify the specific school your target address is assigned to directly with the district — this matters for both day-to-day life and resale.`,`${found}'s public schools fall under a specific BC school district — check the Ministry of Education's district directory. Always verify the exact catchment for a given address with the district before writing an offer, since catchment boundaries change and directly influence a home's resale price.`]),"citation":{"@type":"CreativeWork","name":"BC Ministry of Education — School District Directory","url":"https://www2.gov.bc.ca/gov/content/education-training/k-12/administration/program-management/school-districts"}}},
      {"@type":"Question","name":_v(9,[`What's the best time of year to buy a home in ${found}, BC?`,`When should I start looking for homes in ${found}?`,`Is there a seasonal pattern to ${found} home sales?`]),"acceptedAnswer":{"@type":"Answer","text":_v(109,[`Historically, BC inventory peaks in spring (April-June) and thins out in December-January. You can check current active-listing counts and median list prices for ${found} on the community page — comparing today's numbers against the seasonal average is more useful than the calendar alone.`,`Across BC, listings typically surge April-June and drop off in mid-winter. Whether that pattern is playing out in ${found} right now is what actually matters — the ${found} community page shows the live active-listing count and median list price so you can see if inventory is normal for the season.`,`Spring is BC's traditional listing peak and winter is the trough — but ${found} may or may not be following that pattern in any given year. Open the ${found} community page for the live listing count and median price; if you want a professional interpretation for your situation, reach out to Doug LeMaire, REALTOR® directly.`]),"citation":{"@type":"CreativeWork","name":"BCREA — Housing Market Intelligence","url":"https://www.bcrea.bc.ca/economics/"}}}
    ]
  } : null;
